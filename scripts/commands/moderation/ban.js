...
export function ban(message, args) {
    ...
    if(member.id === player.id) return player.sendMessage(`§r§uRosh §j> §cYou cannot ban yourself.`);
   
    if(member.hasTag("op")) return player.sendMessage(`§r§uRosh §j> §cYou cannot ban other staff members.`);
  
    member.getTags().forEach(t => {
        if(t.includes("reason:") || t.includes("time:")) member.removeTag(t);
    });
