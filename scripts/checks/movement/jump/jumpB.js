...
export function jump_b(player) {
    ...
    if(config.modules.invalidjumpB.enabled) {
        ...
        if(inAir(player) === true && aroundAir(player) === true && player.isJumping && !player.isOnGround && !player.isHoldingTrident && !player.getEffect("jump_boost") && !player.isOnSlime && !player.hasTag("damaged")) {
