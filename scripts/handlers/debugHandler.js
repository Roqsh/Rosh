...
export function debugHandler(player, tps) {
    ...
    /** Dev testing stuff */
    //player.onScreenDisplay.setActionBar(`${config.themecolor}Debug §j> §8Ground-Distance: ${player.location.y - player.lastGroundPosition.y > 1.3 ? "§c" : "§8"}${(player.location.y - player.lastGroundPosition.y).toFixed(4)}§8, Jump-Ticks: ${player.ticksSinceJump}`);
